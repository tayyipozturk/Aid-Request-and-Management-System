                    ...
                    if data[1] == "new":
                        ...
                        if targetUser['campaign']:
                            ...
                            if input_addRequest:
                                CampaignService.add_request(
                                    targetUser['watch_monitor'], input_addRequest, targetUser['campaign'],
                                    targetUser['token'])
                            else:
                                targetUser['watch_monitor'].enqueue(
                                    "Error")
                        ...
                        if input_updateRequest:
                            try:
                                CampaignService.update_request(
                                    targetUser['watch_monitor'], input_updateRequest, targetUser['campaign'],
                                    targetUser['token'])
                            except:
                                targetUser['watch_monitor'].enqueue(
                        ...
                        if input_markAvailable:
                            RequestService.mark_available(
                                targetUser['watch_monitor'], input_markAvailable, targetUser['campaign'],
                                targetUser['token'])
                        else:
                        ...
                        input_arrived = re.search(
                            "(?P<token>[a-zA-Z0-9]*) arrived (?P<req_id>[a-zA-Z0-9\-]*) (?P<ma_id>[a-zA-Z0-9\-]*)",
                            input_data)
                        if input_arrived:
...
if __name__ == "__main__":
    asyncio.run(websocket.start_websocket_server())